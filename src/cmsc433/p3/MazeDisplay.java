...
public class MazeDisplay extends JPanel {
	...
	private void drawMaze(Graphics g) {
		...
		for (int draw_y = cellHeight + 1; curr.row < maze.getHeight(); curr = curr
				.move(Direction.SOUTH), draw_y += cellHeight) {
		...
		g.setColor(lookupColor(maze.getColor(curr)));
		g.fillRect(maze.getWidth() / 2 * cellWidth + 1, (maze.getHeight() + 1) * cellHeight, cellWidth - 1,
				cellHeight + 1);
